...
class ConnectomestatsConfig(typing.TypedDict):
    key: str
    """temporarily set the value of an MRtrix config file entry."""
    value: str
    """temporarily set the value of an MRtrix config file entry."""

Connectomestats = typing.TypedDict(
    "Connectomestats",
    {
        ...
        "config": typing.NotRequired[list[ConnectomestatsConfig]],
...
def connectomestats_validate(params: typing.Any) -> None:
    """Validate untrusted parameters. Raises StyxValidationError if `params` is not a valid Connectomestats."""
    ...
    if params.get("config", None) is not None:
        if not isinstance(params["config"], list):
            raise StyxValidationError(f'`config` has the wrong type: Received `{type(params["config"])}` expected `list[ConnectomestatsConfig]`')
...
def connectomestats_cargs(params: Connectomestats, execution: Execution) -> list[str]:
...
def connectomestats_outputs(params: Connectomestats, execution: Execution) -> ConnectomestatsOutputs:
    """Build the ConnectomestatsOutputs object for this tool."""
    root_v: OutputPathType = execution.output_file(".")
    return ConnectomestatsOutputs(
        root=root_v,
    )

def connectomestats_execute(params: Connectomestats, runner: Runner | None = None) -> ConnectomestatsOutputs:
    """
    connectomestats
...
Neuroimage, 2011, 54(3), 2006-19
    ...
    args = connectomestats_cargs(params, execution)
    out = connectomestats_outputs(params, execution)
    execution.run(args)
    return out
