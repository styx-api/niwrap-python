...
def mris_volmask_params(
    ...
    Args:
        io: Positional argument that represents input/output settings.
        ...
        surf_white: Surface root name (e.g. <subject>/surf/$hemi.<NAME> - default\
            value is white).
        surf_pial: Surface root name (e.g. <subject>/surf/$hemi.<NAME> - default\
            value is pial).
        aseg_name: Specify a different name for the aseg input file (default is\
            aseg).
        out_root: Output root name (default is ribbon). Output files would be, for\
            example, mri/ribbon.mgz.
...
def mris_volmask_validate(params: typing.Any) -> None:
    """Validate untrusted parameters. Raises StyxValidationError if `params` is not a valid MrisVolmask."""
...
def mris_volmask_cargs(params: MrisVolmask, execution: Execution) -> list[str]:
...
def mris_volmask_outputs(params: MrisVolmask, execution: Execution) -> MrisVolmaskOutputs:
    """Build the MrisVolmaskOutputs object for this tool."""
    root_v: OutputPathType = execution.output_file(".")
    return MrisVolmaskOutputs(
        root=root_v,
    )

def mris_volmask_execute(params: MrisVolmask, runner: Runner | None = None) -> MrisVolmaskOutputs:
    """
    mris_volmask
    ...
    args = mris_volmask_cargs(params, execution)
    out = mris_volmask_outputs(params, execution)
    execution.run(args)
    return out
