...
def fixelreorient_config_validate(
    params: typing.Any,
) -> None:
    """
...
def fixelreorient_validate(
    ...
    if not isinstance(params["fixel_out"], str):
        raise StyxValidationError(f'`fixel_out` has the wrong type: Received `{type(params.get("fixel_out", None))}` expected `str`')
