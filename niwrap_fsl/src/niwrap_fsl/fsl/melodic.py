# This file was auto generated by Styx.
# Do not edit this file directly.

import typing
import pathlib
from styxdefs import *

MELODIC_METADATA = Metadata(
    id="0e6ee0e215b2ec04ff4aa7674e916c1ec0900630.boutiques",
    name="melodic",
    package="fsl",
    container_image_tag="brainlife/fsl:6.0.4-patched2",
)


_MelodicParamsDictNoTag = typing.TypedDict('_MelodicParamsDictNoTag', {
    "input_file": InputPathType,
    "output_directory": typing.NotRequired[str | None],
    "mask_file": typing.NotRequired[InputPathType | None],
    "dimensionality_reduction": typing.NotRequired[float | None],
    "generate_report": bool,
    "cifti_io": bool,
    "variance_normalization": bool,
    "no_masking": bool,
    "update_masking": bool,
    "no_bet": bool,
    "bg_threshold": typing.NotRequired[float | None],
    "dimest_technique": typing.NotRequired[str | None],
    "separate_variance_normalization": bool,
    "disable_migp": bool,
    "num_internal_eigenmaps": typing.NotRequired[float | None],
    "migp_shuffle": bool,
    "migp_factor": typing.NotRequired[float | None],
    "num_ics": typing.NotRequired[float | None],
    "nonlinearity": typing.NotRequired[str | None],
    "covar_weights": typing.NotRequired[InputPathType | None],
    "eps_error": typing.NotRequired[float | None],
    "eps_rank1_error": typing.NotRequired[float | None],
    "max_iters": typing.NotRequired[float | None],
    "max_restarts": typing.NotRequired[float | None],
    "mm_threshold": typing.NotRequired[float | None],
    "no_mixture_modeling": bool,
    "ic_components_file": typing.NotRequired[InputPathType | None],
    "mixing_matrix_file": typing.NotRequired[InputPathType | None],
    "session_modes_file": typing.NotRequired[InputPathType | None],
    "component_filter": typing.NotRequired[str | None],
    "background_image": typing.NotRequired[InputPathType | None],
    "tr_seconds": typing.NotRequired[float | None],
    "log_power_calc": bool,
    "time_domain_design_matrix": typing.NotRequired[InputPathType | None],
    "time_domain_t_contrast_matrix": typing.NotRequired[InputPathType | None],
    "subject_domain_design_matrix": typing.NotRequired[InputPathType | None],
    "subject_domain_t_contrast_matrix": typing.NotRequired[InputPathType | None],
    "output_unmixing_matrix": bool,
    "output_stats": bool,
    "output_pca": bool,
    "output_whitening": bool,
    "output_original_ics": bool,
    "output_mean_volume": bool,
    "version": bool,
    "copyright": bool,
    "help": bool,
    "debug": bool,
    "report_maps": typing.NotRequired[str | None],
    "keep_meanvol": bool,
})
MelodicParamsDictTagged = typing.TypedDict('MelodicParamsDictTagged', {
    "@type": typing.Literal["fsl/melodic"],
    "input_file": InputPathType,
    "output_directory": typing.NotRequired[str | None],
    "mask_file": typing.NotRequired[InputPathType | None],
    "dimensionality_reduction": typing.NotRequired[float | None],
    "generate_report": bool,
    "cifti_io": bool,
    "variance_normalization": bool,
    "no_masking": bool,
    "update_masking": bool,
    "no_bet": bool,
    "bg_threshold": typing.NotRequired[float | None],
    "dimest_technique": typing.NotRequired[str | None],
    "separate_variance_normalization": bool,
    "disable_migp": bool,
    "num_internal_eigenmaps": typing.NotRequired[float | None],
    "migp_shuffle": bool,
    "migp_factor": typing.NotRequired[float | None],
    "num_ics": typing.NotRequired[float | None],
    "nonlinearity": typing.NotRequired[str | None],
    "covar_weights": typing.NotRequired[InputPathType | None],
    "eps_error": typing.NotRequired[float | None],
    "eps_rank1_error": typing.NotRequired[float | None],
    "max_iters": typing.NotRequired[float | None],
    "max_restarts": typing.NotRequired[float | None],
    "mm_threshold": typing.NotRequired[float | None],
    "no_mixture_modeling": bool,
    "ic_components_file": typing.NotRequired[InputPathType | None],
    "mixing_matrix_file": typing.NotRequired[InputPathType | None],
    "session_modes_file": typing.NotRequired[InputPathType | None],
    "component_filter": typing.NotRequired[str | None],
    "background_image": typing.NotRequired[InputPathType | None],
    "tr_seconds": typing.NotRequired[float | None],
    "log_power_calc": bool,
    "time_domain_design_matrix": typing.NotRequired[InputPathType | None],
    "time_domain_t_contrast_matrix": typing.NotRequired[InputPathType | None],
    "subject_domain_design_matrix": typing.NotRequired[InputPathType | None],
    "subject_domain_t_contrast_matrix": typing.NotRequired[InputPathType | None],
    "output_unmixing_matrix": bool,
    "output_stats": bool,
    "output_pca": bool,
    "output_whitening": bool,
    "output_original_ics": bool,
    "output_mean_volume": bool,
    "version": bool,
    "copyright": bool,
    "help": bool,
    "debug": bool,
    "report_maps": typing.NotRequired[str | None],
    "keep_meanvol": bool,
})
MelodicParamsDict = _MelodicParamsDictNoTag | MelodicParamsDictTagged


class MelodicOutputs(typing.NamedTuple):
    """
    Output object returned when calling `MelodicParamsDict(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""
    report_file: OutputPathType | None
    """Output Melodic web report"""
    ics_output_file: OutputPathType | None
    """Output IC components file"""
    mix_output_file: OutputPathType | None
    """Output mixing matrix file"""
    temporal_mode_file: OutputPathType | None
    """Output matrix of temporal modes"""
    melodic_report_directory: OutputPathType | None
    """Output Melodic report directory"""


def melodic_params(
    input_file: InputPathType,
    output_directory: str | None = None,
    mask_file: InputPathType | None = None,
    dimensionality_reduction: float | None = None,
    generate_report: bool = False,
    cifti_io: bool = False,
    variance_normalization: bool = False,
    no_masking: bool = False,
    update_masking: bool = False,
    no_bet: bool = False,
    bg_threshold: float | None = None,
    dimest_technique: str | None = None,
    separate_variance_normalization: bool = False,
    disable_migp: bool = False,
    num_internal_eigenmaps: float | None = None,
    migp_shuffle: bool = False,
    migp_factor: float | None = None,
    num_ics: float | None = None,
    nonlinearity: str | None = None,
    covar_weights: InputPathType | None = None,
    eps_error: float | None = None,
    eps_rank1_error: float | None = None,
    max_iters: float | None = None,
    max_restarts: float | None = None,
    mm_threshold: float | None = None,
    no_mixture_modeling: bool = False,
    ic_components_file: InputPathType | None = None,
    mixing_matrix_file: InputPathType | None = None,
    session_modes_file: InputPathType | None = None,
    component_filter: str | None = None,
    background_image: InputPathType | None = None,
    tr_seconds: float | None = None,
    log_power_calc: bool = False,
    time_domain_design_matrix: InputPathType | None = None,
    time_domain_t_contrast_matrix: InputPathType | None = None,
    subject_domain_design_matrix: InputPathType | None = None,
    subject_domain_t_contrast_matrix: InputPathType | None = None,
    output_unmixing_matrix: bool = False,
    output_stats: bool = False,
    output_pca: bool = False,
    output_whitening: bool = False,
    output_original_ics: bool = False,
    output_mean_volume: bool = False,
    version: bool = False,
    copyright_: bool = False,
    help_: bool = False,
    debug: bool = False,
    report_maps: str | None = None,
    keep_meanvol: bool = False,
) -> MelodicParamsDictTagged:
    """
    Build parameters.
    
    Args:
        input_file: Input file names (either single file name or\
            comma-separated list or text file).
        output_directory: Output directory name.
        mask_file: File name of mask for thresholding.
        dimensionality_reduction: Dimensionality reduction into specified\
            number of dimensions (default is automatic estimation).
        generate_report: Generate Melodic web report.
        cifti_io: Input/output as CIFTI (warning: auto-dimensionality\
            estimation for CIFTI data is currently inaccurate).
        variance_normalization: Switch off variance normalization.
        no_masking: Switch off masking.
        update_masking: Switch off mask updating.
        no_bet: Switch off BET.
        bg_threshold: Brain / non-brain threshold (only if --nobet selected).
        dimest_technique: Use specific dimensionality estimation technique:\
            lap, bic, mdl, aic, mean (default: lap).
        separate_variance_normalization: Switch on separate variance\
            normalization for each input dataset (off by default).
        disable_migp: Switch off MIGP data reduction when using -a concat (full\
            temporal concatenation will be used).
        num_internal_eigenmaps: Number of internal Eigenmaps.
        migp_shuffle: Randomize MIGP file order (default: TRUE).
        migp_factor: Internal Factor of mem-threshold relative to number of\
            Eigenmaps (default: 2).
        num_ics: Number of ICs to extract (for deflation approach).
        nonlinearity: Nonlinearity: gauss, tanh, pow3 (default), pow4.
        covar_weights: Voxel-wise weights for the covariance matrix (e.g.\
            segmentation information).
        eps_error: Minimum error change.
        eps_rank1_error: Minimum error change for rank-1 approximation in TICA.
        max_iters: Maximum number of iterations before restart.
        max_restarts: Maximum number of restarts.
        mm_threshold: Threshold for Mixture Model based inference.
        no_mixture_modeling: Switch off mixture modeling on IC maps.
        ic_components_file: Input filename of the IC components file for\
            mixture modeling.
        mixing_matrix_file: Input filename of mixing matrix for mixture\
            modeling / filtering.
        session_modes_file: Input filename of matrix of session modes for\
            report generation.
        component_filter: List of component numbers to remove.
        background_image: Specify background image for report (default: mean\
            image).
        tr_seconds: TR in seconds.
        log_power_calc: Calculate log of power for frequency spectrum.
        time_domain_design_matrix: Design matrix across time-domain.
        time_domain_t_contrast_matrix: T-contrast matrix across time-domain.
        subject_domain_design_matrix: Design matrix across subject-domain.
        subject_domain_t_contrast_matrix: T-contrast matrix across\
            subject-domain.
        output_unmixing_matrix: Output unmixing matrix.
        output_stats: Output thresholded maps and probability maps.
        output_pca: Output PCA results.
        output_whitening: Output whitening/dewhitening matrices.
        output_original_ics: Output the original ICs.
        output_mean_volume: Output mean volume.
        version: Prints version information.
        copyright_: Prints copyright information.
        help_: Prints this help message.
        debug: Switch on debug messages.
        report_maps: Control string for spatial map images (see slicer).
        keep_meanvol: Do not subtract mean volume.
    Returns:
        Parameter dictionary
    """
    params = {
        "@type": "fsl/melodic",
        "input_file": input_file,
        "generate_report": generate_report,
        "cifti_io": cifti_io,
        "variance_normalization": variance_normalization,
        "no_masking": no_masking,
        "update_masking": update_masking,
        "no_bet": no_bet,
        "separate_variance_normalization": separate_variance_normalization,
        "disable_migp": disable_migp,
        "migp_shuffle": migp_shuffle,
        "no_mixture_modeling": no_mixture_modeling,
        "log_power_calc": log_power_calc,
        "output_unmixing_matrix": output_unmixing_matrix,
        "output_stats": output_stats,
        "output_pca": output_pca,
        "output_whitening": output_whitening,
        "output_original_ics": output_original_ics,
        "output_mean_volume": output_mean_volume,
        "version": version,
        "copyright": copyright_,
        "help": help_,
        "debug": debug,
        "keep_meanvol": keep_meanvol,
    }
    if output_directory is not None:
        params["output_directory"] = output_directory
    if mask_file is not None:
        params["mask_file"] = mask_file
    if dimensionality_reduction is not None:
        params["dimensionality_reduction"] = dimensionality_reduction
    if bg_threshold is not None:
        params["bg_threshold"] = bg_threshold
    if dimest_technique is not None:
        params["dimest_technique"] = dimest_technique
    if num_internal_eigenmaps is not None:
        params["num_internal_eigenmaps"] = num_internal_eigenmaps
    if migp_factor is not None:
        params["migp_factor"] = migp_factor
    if num_ics is not None:
        params["num_ics"] = num_ics
    if nonlinearity is not None:
        params["nonlinearity"] = nonlinearity
    if covar_weights is not None:
        params["covar_weights"] = covar_weights
    if eps_error is not None:
        params["eps_error"] = eps_error
    if eps_rank1_error is not None:
        params["eps_rank1_error"] = eps_rank1_error
    if max_iters is not None:
        params["max_iters"] = max_iters
    if max_restarts is not None:
        params["max_restarts"] = max_restarts
    if mm_threshold is not None:
        params["mm_threshold"] = mm_threshold
    if ic_components_file is not None:
        params["ic_components_file"] = ic_components_file
    if mixing_matrix_file is not None:
        params["mixing_matrix_file"] = mixing_matrix_file
    if session_modes_file is not None:
        params["session_modes_file"] = session_modes_file
    if component_filter is not None:
        params["component_filter"] = component_filter
    if background_image is not None:
        params["background_image"] = background_image
    if tr_seconds is not None:
        params["tr_seconds"] = tr_seconds
    if time_domain_design_matrix is not None:
        params["time_domain_design_matrix"] = time_domain_design_matrix
    if time_domain_t_contrast_matrix is not None:
        params["time_domain_t_contrast_matrix"] = time_domain_t_contrast_matrix
    if subject_domain_design_matrix is not None:
        params["subject_domain_design_matrix"] = subject_domain_design_matrix
    if subject_domain_t_contrast_matrix is not None:
        params["subject_domain_t_contrast_matrix"] = subject_domain_t_contrast_matrix
    if report_maps is not None:
        params["report_maps"] = report_maps
    return params


def melodic_validate(
    params: typing.Any,
) -> None:
    """
    Validate parameters. Throws an error if `params` is not a valid
    `MelodicParamsDict` object.
    
    Args:
        params: The parameters object to validate.
    """
    if params is None or not isinstance(params, dict):
        raise StyxValidationError(f'Params object has the wrong type \'{type(params)}\'')
    if params.get("input_file", None) is None:
        raise StyxValidationError("`input_file` must not be None")
    if not isinstance(params["input_file"], (pathlib.Path, str)):
        raise StyxValidationError(f'`input_file` has the wrong type: Received `{type(params.get("input_file", None))}` expected `InputPathType`')
    if params.get("output_directory", None) is not None:
        if not isinstance(params["output_directory"], str):
            raise StyxValidationError(f'`output_directory` has the wrong type: Received `{type(params.get("output_directory", None))}` expected `str | None`')
    if params.get("mask_file", None) is not None:
        if not isinstance(params["mask_file"], (pathlib.Path, str)):
            raise StyxValidationError(f'`mask_file` has the wrong type: Received `{type(params.get("mask_file", None))}` expected `InputPathType | None`')
    if params.get("dimensionality_reduction", None) is not None:
        if not isinstance(params["dimensionality_reduction"], (float, int)):
            raise StyxValidationError(f'`dimensionality_reduction` has the wrong type: Received `{type(params.get("dimensionality_reduction", None))}` expected `float | None`')
    if params.get("generate_report", False) is None:
        raise StyxValidationError("`generate_report` must not be None")
    if not isinstance(params["generate_report"], bool):
        raise StyxValidationError(f'`generate_report` has the wrong type: Received `{type(params.get("generate_report", False))}` expected `bool`')
    if params.get("cifti_io", False) is None:
        raise StyxValidationError("`cifti_io` must not be None")
    if not isinstance(params["cifti_io"], bool):
        raise StyxValidationError(f'`cifti_io` has the wrong type: Received `{type(params.get("cifti_io", False))}` expected `bool`')
    if params.get("variance_normalization", False) is None:
        raise StyxValidationError("`variance_normalization` must not be None")
    if not isinstance(params["variance_normalization"], bool):
        raise StyxValidationError(f'`variance_normalization` has the wrong type: Received `{type(params.get("variance_normalization", False))}` expected `bool`')
    if params.get("no_masking", False) is None:
        raise StyxValidationError("`no_masking` must not be None")
    if not isinstance(params["no_masking"], bool):
        raise StyxValidationError(f'`no_masking` has the wrong type: Received `{type(params.get("no_masking", False))}` expected `bool`')
    if params.get("update_masking", False) is None:
        raise StyxValidationError("`update_masking` must not be None")
    if not isinstance(params["update_masking"], bool):
        raise StyxValidationError(f'`update_masking` has the wrong type: Received `{type(params.get("update_masking", False))}` expected `bool`')
    if params.get("no_bet", False) is None:
        raise StyxValidationError("`no_bet` must not be None")
    if not isinstance(params["no_bet"], bool):
        raise StyxValidationError(f'`no_bet` has the wrong type: Received `{type(params.get("no_bet", False))}` expected `bool`')
    if params.get("bg_threshold", None) is not None:
        if not isinstance(params["bg_threshold"], (float, int)):
            raise StyxValidationError(f'`bg_threshold` has the wrong type: Received `{type(params.get("bg_threshold", None))}` expected `float | None`')
    if params.get("dimest_technique", None) is not None:
        if not isinstance(params["dimest_technique"], str):
            raise StyxValidationError(f'`dimest_technique` has the wrong type: Received `{type(params.get("dimest_technique", None))}` expected `str | None`')
    if params.get("separate_variance_normalization", False) is None:
        raise StyxValidationError("`separate_variance_normalization` must not be None")
    if not isinstance(params["separate_variance_normalization"], bool):
        raise StyxValidationError(f'`separate_variance_normalization` has the wrong type: Received `{type(params.get("separate_variance_normalization", False))}` expected `bool`')
    if params.get("disable_migp", False) is None:
        raise StyxValidationError("`disable_migp` must not be None")
    if not isinstance(params["disable_migp"], bool):
        raise StyxValidationError(f'`disable_migp` has the wrong type: Received `{type(params.get("disable_migp", False))}` expected `bool`')
    if params.get("num_internal_eigenmaps", None) is not None:
        if not isinstance(params["num_internal_eigenmaps"], (float, int)):
            raise StyxValidationError(f'`num_internal_eigenmaps` has the wrong type: Received `{type(params.get("num_internal_eigenmaps", None))}` expected `float | None`')
    if params.get("migp_shuffle", False) is None:
        raise StyxValidationError("`migp_shuffle` must not be None")
    if not isinstance(params["migp_shuffle"], bool):
        raise StyxValidationError(f'`migp_shuffle` has the wrong type: Received `{type(params.get("migp_shuffle", False))}` expected `bool`')
    if params.get("migp_factor", None) is not None:
        if not isinstance(params["migp_factor"], (float, int)):
            raise StyxValidationError(f'`migp_factor` has the wrong type: Received `{type(params.get("migp_factor", None))}` expected `float | None`')
    if params.get("num_ics", None) is not None:
        if not isinstance(params["num_ics"], (float, int)):
            raise StyxValidationError(f'`num_ics` has the wrong type: Received `{type(params.get("num_ics", None))}` expected `float | None`')
    if params.get("nonlinearity", None) is not None:
        if not isinstance(params["nonlinearity"], str):
            raise StyxValidationError(f'`nonlinearity` has the wrong type: Received `{type(params.get("nonlinearity", None))}` expected `str | None`')
    if params.get("covar_weights", None) is not None:
        if not isinstance(params["covar_weights"], (pathlib.Path, str)):
            raise StyxValidationError(f'`covar_weights` has the wrong type: Received `{type(params.get("covar_weights", None))}` expected `InputPathType | None`')
    if params.get("eps_error", None) is not None:
        if not isinstance(params["eps_error"], (float, int)):
            raise StyxValidationError(f'`eps_error` has the wrong type: Received `{type(params.get("eps_error", None))}` expected `float | None`')
    if params.get("eps_rank1_error", None) is not None:
        if not isinstance(params["eps_rank1_error"], (float, int)):
            raise StyxValidationError(f'`eps_rank1_error` has the wrong type: Received `{type(params.get("eps_rank1_error", None))}` expected `float | None`')
    if params.get("max_iters", None) is not None:
        if not isinstance(params["max_iters"], (float, int)):
            raise StyxValidationError(f'`max_iters` has the wrong type: Received `{type(params.get("max_iters", None))}` expected `float | None`')
    if params.get("max_restarts", None) is not None:
        if not isinstance(params["max_restarts"], (float, int)):
            raise StyxValidationError(f'`max_restarts` has the wrong type: Received `{type(params.get("max_restarts", None))}` expected `float | None`')
    if params.get("mm_threshold", None) is not None:
        if not isinstance(params["mm_threshold"], (float, int)):
            raise StyxValidationError(f'`mm_threshold` has the wrong type: Received `{type(params.get("mm_threshold", None))}` expected `float | None`')
    if params.get("no_mixture_modeling", False) is None:
        raise StyxValidationError("`no_mixture_modeling` must not be None")
    if not isinstance(params["no_mixture_modeling"], bool):
        raise StyxValidationError(f'`no_mixture_modeling` has the wrong type: Received `{type(params.get("no_mixture_modeling", False))}` expected `bool`')
    if params.get("ic_components_file", None) is not None:
        if not isinstance(params["ic_components_file"], (pathlib.Path, str)):
            raise StyxValidationError(f'`ic_components_file` has the wrong type: Received `{type(params.get("ic_components_file", None))}` expected `InputPathType | None`')
    if params.get("mixing_matrix_file", None) is not None:
        if not isinstance(params["mixing_matrix_file"], (pathlib.Path, str)):
            raise StyxValidationError(f'`mixing_matrix_file` has the wrong type: Received `{type(params.get("mixing_matrix_file", None))}` expected `InputPathType | None`')
    if params.get("session_modes_file", None) is not None:
        if not isinstance(params["session_modes_file"], (pathlib.Path, str)):
            raise StyxValidationError(f'`session_modes_file` has the wrong type: Received `{type(params.get("session_modes_file", None))}` expected `InputPathType | None`')
    if params.get("component_filter", None) is not None:
        if not isinstance(params["component_filter"], str):
            raise StyxValidationError(f'`component_filter` has the wrong type: Received `{type(params.get("component_filter", None))}` expected `str | None`')
    if params.get("background_image", None) is not None:
        if not isinstance(params["background_image"], (pathlib.Path, str)):
            raise StyxValidationError(f'`background_image` has the wrong type: Received `{type(params.get("background_image", None))}` expected `InputPathType | None`')
    if params.get("tr_seconds", None) is not None:
        if not isinstance(params["tr_seconds"], (float, int)):
            raise StyxValidationError(f'`tr_seconds` has the wrong type: Received `{type(params.get("tr_seconds", None))}` expected `float | None`')
    if params.get("log_power_calc", False) is None:
        raise StyxValidationError("`log_power_calc` must not be None")
    if not isinstance(params["log_power_calc"], bool):
        raise StyxValidationError(f'`log_power_calc` has the wrong type: Received `{type(params.get("log_power_calc", False))}` expected `bool`')
    if params.get("time_domain_design_matrix", None) is not None:
        if not isinstance(params["time_domain_design_matrix"], (pathlib.Path, str)):
            raise StyxValidationError(f'`time_domain_design_matrix` has the wrong type: Received `{type(params.get("time_domain_design_matrix", None))}` expected `InputPathType | None`')
    if params.get("time_domain_t_contrast_matrix", None) is not None:
        if not isinstance(params["time_domain_t_contrast_matrix"], (pathlib.Path, str)):
            raise StyxValidationError(f'`time_domain_t_contrast_matrix` has the wrong type: Received `{type(params.get("time_domain_t_contrast_matrix", None))}` expected `InputPathType | None`')
    if params.get("subject_domain_design_matrix", None) is not None:
        if not isinstance(params["subject_domain_design_matrix"], (pathlib.Path, str)):
            raise StyxValidationError(f'`subject_domain_design_matrix` has the wrong type: Received `{type(params.get("subject_domain_design_matrix", None))}` expected `InputPathType | None`')
    if params.get("subject_domain_t_contrast_matrix", None) is not None:
        if not isinstance(params["subject_domain_t_contrast_matrix"], (pathlib.Path, str)):
            raise StyxValidationError(f'`subject_domain_t_contrast_matrix` has the wrong type: Received `{type(params.get("subject_domain_t_contrast_matrix", None))}` expected `InputPathType | None`')
    if params.get("output_unmixing_matrix", False) is None:
        raise StyxValidationError("`output_unmixing_matrix` must not be None")
    if not isinstance(params["output_unmixing_matrix"], bool):
        raise StyxValidationError(f'`output_unmixing_matrix` has the wrong type: Received `{type(params.get("output_unmixing_matrix", False))}` expected `bool`')
    if params.get("output_stats", False) is None:
        raise StyxValidationError("`output_stats` must not be None")
    if not isinstance(params["output_stats"], bool):
        raise StyxValidationError(f'`output_stats` has the wrong type: Received `{type(params.get("output_stats", False))}` expected `bool`')
    if params.get("output_pca", False) is None:
        raise StyxValidationError("`output_pca` must not be None")
    if not isinstance(params["output_pca"], bool):
        raise StyxValidationError(f'`output_pca` has the wrong type: Received `{type(params.get("output_pca", False))}` expected `bool`')
    if params.get("output_whitening", False) is None:
        raise StyxValidationError("`output_whitening` must not be None")
    if not isinstance(params["output_whitening"], bool):
        raise StyxValidationError(f'`output_whitening` has the wrong type: Received `{type(params.get("output_whitening", False))}` expected `bool`')
    if params.get("output_original_ics", False) is None:
        raise StyxValidationError("`output_original_ics` must not be None")
    if not isinstance(params["output_original_ics"], bool):
        raise StyxValidationError(f'`output_original_ics` has the wrong type: Received `{type(params.get("output_original_ics", False))}` expected `bool`')
    if params.get("output_mean_volume", False) is None:
        raise StyxValidationError("`output_mean_volume` must not be None")
    if not isinstance(params["output_mean_volume"], bool):
        raise StyxValidationError(f'`output_mean_volume` has the wrong type: Received `{type(params.get("output_mean_volume", False))}` expected `bool`')
    if params.get("version", False) is None:
        raise StyxValidationError("`version` must not be None")
    if not isinstance(params["version"], bool):
        raise StyxValidationError(f'`version` has the wrong type: Received `{type(params.get("version", False))}` expected `bool`')
    if params.get("copyright", False) is None:
        raise StyxValidationError("`copyright` must not be None")
    if not isinstance(params["copyright"], bool):
        raise StyxValidationError(f'`copyright` has the wrong type: Received `{type(params.get("copyright", False))}` expected `bool`')
    if params.get("help", False) is None:
        raise StyxValidationError("`help` must not be None")
    if not isinstance(params["help"], bool):
        raise StyxValidationError(f'`help` has the wrong type: Received `{type(params.get("help", False))}` expected `bool`')
    if params.get("debug", False) is None:
        raise StyxValidationError("`debug` must not be None")
    if not isinstance(params["debug"], bool):
        raise StyxValidationError(f'`debug` has the wrong type: Received `{type(params.get("debug", False))}` expected `bool`')
    if params.get("report_maps", None) is not None:
        if not isinstance(params["report_maps"], str):
            raise StyxValidationError(f'`report_maps` has the wrong type: Received `{type(params.get("report_maps", None))}` expected `str | None`')
    if params.get("keep_meanvol", False) is None:
        raise StyxValidationError("`keep_meanvol` must not be None")
    if not isinstance(params["keep_meanvol"], bool):
        raise StyxValidationError(f'`keep_meanvol` has the wrong type: Received `{type(params.get("keep_meanvol", False))}` expected `bool`')


def melodic_cargs(
    params: MelodicParamsDict,
    execution: Execution,
) -> list[str]:
    """
    Build command-line arguments from parameters.
    
    Args:
        params: The parameters.
        execution: The execution object for resolving input paths.
    Returns:
        Command-line arguments.
    """
    cargs = []
    cargs.append("melodic")
    cargs.extend([
        "-i",
        execution.input_file(params.get("input_file", None))
    ])
    if params.get("output_directory", None) is not None:
        cargs.extend([
            "-o",
            params.get("output_directory", None)
        ])
    if params.get("mask_file", None) is not None:
        cargs.extend([
            "-m",
            execution.input_file(params.get("mask_file", None))
        ])
    if params.get("dimensionality_reduction", None) is not None:
        cargs.extend([
            "-d",
            str(params.get("dimensionality_reduction", None))
        ])
    if params.get("generate_report", False):
        cargs.append("--report")
    if params.get("cifti_io", False):
        cargs.append("--CIFTI")
    if params.get("variance_normalization", False):
        cargs.append("--vn")
    if params.get("no_masking", False):
        cargs.append("--nomask")
    if params.get("update_masking", False):
        cargs.append("--update_mask")
    if params.get("no_bet", False):
        cargs.append("--nobet")
    if params.get("bg_threshold", None) is not None:
        cargs.extend([
            "--bgthreshold",
            str(params.get("bg_threshold", None))
        ])
    if params.get("dimest_technique", None) is not None:
        cargs.extend([
            "--dimest",
            params.get("dimest_technique", None)
        ])
    if params.get("separate_variance_normalization", False):
        cargs.append("--sep_vn")
    if params.get("disable_migp", False):
        cargs.append("--disableMigp")
    if params.get("num_internal_eigenmaps", None) is not None:
        cargs.extend([
            "--migpN",
            str(params.get("num_internal_eigenmaps", None))
        ])
    if params.get("migp_shuffle", False):
        cargs.append("--migp_shuffle")
    if params.get("migp_factor", None) is not None:
        cargs.extend([
            "--migp_factor",
            str(params.get("migp_factor", None))
        ])
    if params.get("num_ics", None) is not None:
        cargs.extend([
            "-n",
            str(params.get("num_ics", None))
        ])
    if params.get("nonlinearity", None) is not None:
        cargs.extend([
            "--nl",
            params.get("nonlinearity", None)
        ])
    if params.get("covar_weights", None) is not None:
        cargs.extend([
            "--covarweight",
            execution.input_file(params.get("covar_weights", None))
        ])
    if params.get("eps_error", None) is not None:
        cargs.extend([
            "--eps",
            str(params.get("eps_error", None))
        ])
    if params.get("eps_rank1_error", None) is not None:
        cargs.extend([
            "--epsS",
            str(params.get("eps_rank1_error", None))
        ])
    if params.get("max_iters", None) is not None:
        cargs.extend([
            "--maxit",
            str(params.get("max_iters", None))
        ])
    if params.get("max_restarts", None) is not None:
        cargs.extend([
            "--maxrestart",
            str(params.get("max_restarts", None))
        ])
    if params.get("mm_threshold", None) is not None:
        cargs.extend([
            "--mmthresh",
            str(params.get("mm_threshold", None))
        ])
    if params.get("no_mixture_modeling", False):
        cargs.append("--no_mm")
    if params.get("ic_components_file", None) is not None:
        cargs.extend([
            "--ICs",
            execution.input_file(params.get("ic_components_file", None))
        ])
    if params.get("mixing_matrix_file", None) is not None:
        cargs.extend([
            "--mix",
            execution.input_file(params.get("mixing_matrix_file", None))
        ])
    if params.get("session_modes_file", None) is not None:
        cargs.extend([
            "--smode",
            execution.input_file(params.get("session_modes_file", None))
        ])
    if params.get("component_filter", None) is not None:
        cargs.extend([
            "-f",
            params.get("component_filter", None)
        ])
    if params.get("background_image", None) is not None:
        cargs.extend([
            "--bgimage",
            execution.input_file(params.get("background_image", None))
        ])
    if params.get("tr_seconds", None) is not None:
        cargs.extend([
            "--tr",
            str(params.get("tr_seconds", None))
        ])
    if params.get("log_power_calc", False):
        cargs.append("--logPower")
    if params.get("time_domain_design_matrix", None) is not None:
        cargs.extend([
            "--Tdes",
            execution.input_file(params.get("time_domain_design_matrix", None))
        ])
    if params.get("time_domain_t_contrast_matrix", None) is not None:
        cargs.extend([
            "--Tcon",
            execution.input_file(params.get("time_domain_t_contrast_matrix", None))
        ])
    if params.get("subject_domain_design_matrix", None) is not None:
        cargs.extend([
            "--Sdes",
            execution.input_file(params.get("subject_domain_design_matrix", None))
        ])
    if params.get("subject_domain_t_contrast_matrix", None) is not None:
        cargs.extend([
            "--Scon",
            execution.input_file(params.get("subject_domain_t_contrast_matrix", None))
        ])
    if params.get("output_unmixing_matrix", False):
        cargs.append("--Ounmix")
    if params.get("output_stats", False):
        cargs.append("--Ostats")
    if params.get("output_pca", False):
        cargs.append("--Opca")
    if params.get("output_whitening", False):
        cargs.append("--Owhite")
    if params.get("output_original_ics", False):
        cargs.append("--Oorig")
    if params.get("output_mean_volume", False):
        cargs.append("--Omean")
    if params.get("version", False):
        cargs.append("-V")
    if params.get("copyright", False):
        cargs.append("--copyright")
    if params.get("help", False):
        cargs.append("-h")
    if params.get("debug", False):
        cargs.append("--debug")
    if params.get("report_maps", None) is not None:
        cargs.extend([
            "--report_maps",
            params.get("report_maps", None)
        ])
    if params.get("keep_meanvol", False):
        cargs.append("--keep_meanvol")
    return cargs


def melodic_outputs(
    params: MelodicParamsDict,
    execution: Execution,
) -> MelodicOutputs:
    """
    Build outputs object containing output file paths and possibly stdout/stderr.
    
    Args:
        params: The parameters.
        execution: The execution object for resolving input paths.
    Returns:
        Outputs object.
    """
    ret = MelodicOutputs(
        root=execution.output_file("."),
        report_file=execution.output_file(params.get("output_directory", None) + "/report.html") if (params.get("output_directory") is not None) else None,
        ics_output_file=execution.output_file(params.get("output_directory", None) + "/melodic_IC.nii.gz") if (params.get("output_directory") is not None) else None,
        mix_output_file=execution.output_file(params.get("output_directory", None) + "/melodic_mix") if (params.get("output_directory") is not None) else None,
        temporal_mode_file=execution.output_file(params.get("output_directory", None) + "/melodic_Tmodes") if (params.get("output_directory") is not None) else None,
        melodic_report_directory=execution.output_file(params.get("output_directory", None) + "/melodic_report") if (params.get("output_directory") is not None) else None,
    )
    return ret


def melodic_execute(
    params: MelodicParamsDict,
    runner: Runner | None = None,
) -> MelodicOutputs:
    """
    melodic
    
    Multivariate Exploratory Linear Optimised Decomposition into Independent
    Components.
    
    Author: FMRIB Analysis Group, University of Oxford
    
    URL: https://fsl.fmrib.ox.ac.uk/fsl/fslwiki
    
    Args:
        params: The parameters.
        runner: Command runner.
    Returns:
        NamedTuple of outputs (described in `MelodicOutputs`).
    """
    melodic_validate(params)
    runner = runner or get_global_runner()
    execution = runner.start_execution(MELODIC_METADATA)
    params = execution.params(params)
    cargs = melodic_cargs(params, execution)
    ret = melodic_outputs(params, execution)
    execution.run(cargs)
    return ret


def melodic(
    input_file: InputPathType,
    output_directory: str | None = None,
    mask_file: InputPathType | None = None,
    dimensionality_reduction: float | None = None,
    generate_report: bool = False,
    cifti_io: bool = False,
    variance_normalization: bool = False,
    no_masking: bool = False,
    update_masking: bool = False,
    no_bet: bool = False,
    bg_threshold: float | None = None,
    dimest_technique: str | None = None,
    separate_variance_normalization: bool = False,
    disable_migp: bool = False,
    num_internal_eigenmaps: float | None = None,
    migp_shuffle: bool = False,
    migp_factor: float | None = None,
    num_ics: float | None = None,
    nonlinearity: str | None = None,
    covar_weights: InputPathType | None = None,
    eps_error: float | None = None,
    eps_rank1_error: float | None = None,
    max_iters: float | None = None,
    max_restarts: float | None = None,
    mm_threshold: float | None = None,
    no_mixture_modeling: bool = False,
    ic_components_file: InputPathType | None = None,
    mixing_matrix_file: InputPathType | None = None,
    session_modes_file: InputPathType | None = None,
    component_filter: str | None = None,
    background_image: InputPathType | None = None,
    tr_seconds: float | None = None,
    log_power_calc: bool = False,
    time_domain_design_matrix: InputPathType | None = None,
    time_domain_t_contrast_matrix: InputPathType | None = None,
    subject_domain_design_matrix: InputPathType | None = None,
    subject_domain_t_contrast_matrix: InputPathType | None = None,
    output_unmixing_matrix: bool = False,
    output_stats: bool = False,
    output_pca: bool = False,
    output_whitening: bool = False,
    output_original_ics: bool = False,
    output_mean_volume: bool = False,
    version: bool = False,
    copyright_: bool = False,
    help_: bool = False,
    debug: bool = False,
    report_maps: str | None = None,
    keep_meanvol: bool = False,
    runner: Runner | None = None,
) -> MelodicOutputs:
    """
    melodic
    
    Multivariate Exploratory Linear Optimised Decomposition into Independent
    Components.
    
    Author: FMRIB Analysis Group, University of Oxford
    
    URL: https://fsl.fmrib.ox.ac.uk/fsl/fslwiki
    
    Args:
        input_file: Input file names (either single file name or\
            comma-separated list or text file).
        output_directory: Output directory name.
        mask_file: File name of mask for thresholding.
        dimensionality_reduction: Dimensionality reduction into specified\
            number of dimensions (default is automatic estimation).
        generate_report: Generate Melodic web report.
        cifti_io: Input/output as CIFTI (warning: auto-dimensionality\
            estimation for CIFTI data is currently inaccurate).
        variance_normalization: Switch off variance normalization.
        no_masking: Switch off masking.
        update_masking: Switch off mask updating.
        no_bet: Switch off BET.
        bg_threshold: Brain / non-brain threshold (only if --nobet selected).
        dimest_technique: Use specific dimensionality estimation technique:\
            lap, bic, mdl, aic, mean (default: lap).
        separate_variance_normalization: Switch on separate variance\
            normalization for each input dataset (off by default).
        disable_migp: Switch off MIGP data reduction when using -a concat (full\
            temporal concatenation will be used).
        num_internal_eigenmaps: Number of internal Eigenmaps.
        migp_shuffle: Randomize MIGP file order (default: TRUE).
        migp_factor: Internal Factor of mem-threshold relative to number of\
            Eigenmaps (default: 2).
        num_ics: Number of ICs to extract (for deflation approach).
        nonlinearity: Nonlinearity: gauss, tanh, pow3 (default), pow4.
        covar_weights: Voxel-wise weights for the covariance matrix (e.g.\
            segmentation information).
        eps_error: Minimum error change.
        eps_rank1_error: Minimum error change for rank-1 approximation in TICA.
        max_iters: Maximum number of iterations before restart.
        max_restarts: Maximum number of restarts.
        mm_threshold: Threshold for Mixture Model based inference.
        no_mixture_modeling: Switch off mixture modeling on IC maps.
        ic_components_file: Input filename of the IC components file for\
            mixture modeling.
        mixing_matrix_file: Input filename of mixing matrix for mixture\
            modeling / filtering.
        session_modes_file: Input filename of matrix of session modes for\
            report generation.
        component_filter: List of component numbers to remove.
        background_image: Specify background image for report (default: mean\
            image).
        tr_seconds: TR in seconds.
        log_power_calc: Calculate log of power for frequency spectrum.
        time_domain_design_matrix: Design matrix across time-domain.
        time_domain_t_contrast_matrix: T-contrast matrix across time-domain.
        subject_domain_design_matrix: Design matrix across subject-domain.
        subject_domain_t_contrast_matrix: T-contrast matrix across\
            subject-domain.
        output_unmixing_matrix: Output unmixing matrix.
        output_stats: Output thresholded maps and probability maps.
        output_pca: Output PCA results.
        output_whitening: Output whitening/dewhitening matrices.
        output_original_ics: Output the original ICs.
        output_mean_volume: Output mean volume.
        version: Prints version information.
        copyright_: Prints copyright information.
        help_: Prints this help message.
        debug: Switch on debug messages.
        report_maps: Control string for spatial map images (see slicer).
        keep_meanvol: Do not subtract mean volume.
        runner: Command runner.
    Returns:
        NamedTuple of outputs (described in `MelodicOutputs`).
    """
    params = melodic_params(
        input_file=input_file,
        output_directory=output_directory,
        mask_file=mask_file,
        dimensionality_reduction=dimensionality_reduction,
        generate_report=generate_report,
        cifti_io=cifti_io,
        variance_normalization=variance_normalization,
        no_masking=no_masking,
        update_masking=update_masking,
        no_bet=no_bet,
        bg_threshold=bg_threshold,
        dimest_technique=dimest_technique,
        separate_variance_normalization=separate_variance_normalization,
        disable_migp=disable_migp,
        num_internal_eigenmaps=num_internal_eigenmaps,
        migp_shuffle=migp_shuffle,
        migp_factor=migp_factor,
        num_ics=num_ics,
        nonlinearity=nonlinearity,
        covar_weights=covar_weights,
        eps_error=eps_error,
        eps_rank1_error=eps_rank1_error,
        max_iters=max_iters,
        max_restarts=max_restarts,
        mm_threshold=mm_threshold,
        no_mixture_modeling=no_mixture_modeling,
        ic_components_file=ic_components_file,
        mixing_matrix_file=mixing_matrix_file,
        session_modes_file=session_modes_file,
        component_filter=component_filter,
        background_image=background_image,
        tr_seconds=tr_seconds,
        log_power_calc=log_power_calc,
        time_domain_design_matrix=time_domain_design_matrix,
        time_domain_t_contrast_matrix=time_domain_t_contrast_matrix,
        subject_domain_design_matrix=subject_domain_design_matrix,
        subject_domain_t_contrast_matrix=subject_domain_t_contrast_matrix,
        output_unmixing_matrix=output_unmixing_matrix,
        output_stats=output_stats,
        output_pca=output_pca,
        output_whitening=output_whitening,
        output_original_ics=output_original_ics,
        output_mean_volume=output_mean_volume,
        version=version,
        copyright_=copyright_,
        help_=help_,
        debug=debug,
        report_maps=report_maps,
        keep_meanvol=keep_meanvol,
    )
    return melodic_execute(params, runner)


__all__ = [
    "MELODIC_METADATA",
    "MelodicOutputs",
    "MelodicParamsDict",
    "MelodicParamsDictTagged",
    "melodic",
    "melodic_execute",
    "melodic_params",
]
